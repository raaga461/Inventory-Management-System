...
import transaction.Reports.CreditNotePdf;
...
@ManagedBean(name = "dbnote", eager = true)
@ViewScoped
...
System.out.println("new currbalance is "+cdb.getCurrbal());
    ...
    public void itemDetails() {
        ...
        String qry = "";
        try {
            qry = "SELECT itemcode,itemname,CATEGORY  from stockpurchase";
            rs = st.executeQuery(qry);
            while (rs.next()) {
                ib = new ItemListBean();
    ...
    public void print(){
        ...
        CreditNotePdf ob=new CreditNotePdf();
    ...
    public void onRowSelect(SelectEvent event) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                cdb.setPhn(rs.getString("phnmobile"));
                cdb.setModify("NO");
                cdb.setSlno(0);
                cdb.setOrno(0);
                cdb.setLrno(0);
    ...
    public void bill() {
       ...
       st = Conn_Statement();
           String qry = "";
           qry = "SELECT max(salesid), max(Billno),max(reffno) FROM debitnotepurchases where cmpid=" + companyid + "";
           try {
               rs = st.executeQuery(qry);
               if (rs.next()) {
...
System.out.println(new java.sql.Date(cdb.getInvdate().getTime()));
        try {
            ...
            String qry1="select * from debitnotepurchases where Billno="+cdb.getBillno() +"";
            st=Conn_Statement();
            ...
            String qry =
                "insert into debitnotepurchases values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
System.out.println("Row size is ---"+tdata.size());
           ...
           ps.setDate(50, new java.sql.Date(cdb.getLrDate().getTime()));
            ...
            ps.setString(69, cdb.getInolist());
   ...
   // String iname = table.getValueAt(count, 1).toString();
    ...
    System.out.println("Row size is ---"+tdata.size());
           ...
           ps.setDate(50, new java.sql.Date(cdb.getLrDate().getTime()));
            ...
            ps=(PreparedStatement)con.prepareStatement("UPDATE Customer SET currbal = " + cdb.getCurrbal()+ " WHERE acccode="+cdb.getAcccode() +"");
            System.out.println("current balance is "+cdb.getCurrbal());
            l=ps.executeUpdate();
System.out.println("values are-----------------"+rows+"      "+j+"              "+k+"                         "+l);
    ...
    public int StoringCurrBal() {
            ...
            try {
                ...
                rs.close();
                st.close();
                qry = "SELECT distinct(invvalue)  FROM  debitnotepurchases WHERE Billno =" +cdb.getBillno();
                System.out.println("" + qry);
                st=Conn_Statement();
                rs = st.executeQuery(qry);
                if (rs.next()) {
...
rs.close();
               ...
               /*  qry = "DELETE FROM debitnotepurchases WHERE Billno =" + cdb.getBillno();
                System.out.println("" + qry);
                st=Conn_Statement();
...
return l;
    ...
    public void onRowSelect2(SelectEvent event) {
       ...
       int z=0;
        ...
        String qry="select * from debitnotepurchases where reffno="+refno+"";
        try{
        st=Conn_Statement();
        rs=st.executeQuery(qry);
            while(rs.next()){
             ...
             cdb.setCname(rs.getString(5));
               ...
               cdb.setInolist(rs.getString(69));
             //   searchttb.seti(rs.getString(69));
    ...
    public void update()  {
        ...
        String qry="delete from debitnotepurchases where Billno="+cdb.getBillno() +"";
    ...
    public void searchCust(){
        st=Conn_Statement();
        String qry="select distinct reffno,acname from debitnotepurchases";

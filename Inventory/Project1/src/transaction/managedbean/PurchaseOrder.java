...
import transaction.Reports.PurchaseOrderPDF;
import transaction.Reports.SalesQuatationsPDF;
...
@ManagedBean(name = "pord", eager = true)
@ViewScoped
...
//con.setAutoCommit(false);
    ...
    public void custDetails() {
        ...
        String qry = "";
        try {
            qry = "SELECT acccode, companyname FROM Vendor";
...
System.out.println("new currbalance is "+cdb.getCurrbal());
    ...
    public void itemDetails() {
        ...
        String qry = "";
        try {
            qry = "SELECT itemcode,itemname,itemrate  from ItemPurchase";
            rs = st.executeQuery(qry);
            while (rs.next()) {
                ib = new ItemListBean();
...
//    } */
        ...
        try {
            ...
            if (cdb.getTaxtype() != null) {
                ...
                if (cdb.getTaxtype().contains("GEN")) {
                    ...
                    cdb.setInvno("PR-ORD/" + cdb.getRefno());
                    C_Form = "NO";
                }
    ...
    public void print(){
        NumToWords ob1=new NumToWords();
         String nw=ob1.NumToWord(dfr.format(cdb.getInvvalue()).toString());
         System.out.println("nw value is "+nw);
        PurchaseOrderPDF ob=new PurchaseOrderPDF();
       ob.execute(cdb.getBillno().toString(), cdb.getInvdate().toString(), nw,cdb.getNote());
    }
    public void onRowSelect(SelectEvent event) {
        ...
        try {
            ...
            qry = " SELECT * FROM Vendor where companyname='" + sname + "'";
            ...
            if (rs.next()) {
                ...
                cdb.setCname(rs.getString("companyname"));
...
//cdb.setSlno(rs.getInt("slno"));
             ...
             //  cdb.setAddr(rs.getString("addr"));
                ...
                cdb.setCate(rs.getString("catogory"));
                cdb.setSubcat(rs.getString("subcat"));
                
              cdb.setPhn(rs.getString("R_phn"));
              cdb.setModify("NO");
               // cdb.setSlno(0);
                cdb.setOrno(0);
              //  cdb.setLrno(0);
               ...
               cdb.setGunybags(0.0);
                cdb.setBundles(0.0);
                System.out.println("dbhdbd=====1111====1==11==1"+cdb.getGunybags()+"faffafafsa"+cdb.getBundles());
                cdb.setNote(".");
    ...
    public void bill() {
        ...
        st = Conn_Statement();
        String qry = "";
        qry = "SELECT max(salesid), max(Billno),max(reffno) FROM PurchaseOrder where cmpid='" + companyid + "'";
        try {
            rs = st.executeQuery(qry);
            if (rs.next()) {
         ...
         /*    st = Conn_Statement();
            rs = st.executeQuery("SELECT max(CFID) FROM PurchaseOrder where cmpid='" + companyid + "'");
            if (rs.next()) {
...
System.out.println(new java.sql.Date(cdb.getInvdate().getTime()));
        try {
            ...
            String qry1="select * from PurchaseOrder where Billno="+cdb.getBillno() +"";
            st=Conn_Statement();
...
System.out.println("Row size is ---"+tdata.size());
            ...
            ps.setInt(17, tdata.get(i).getSs());
            ...
            ps.setString(37, cdb.getCate());
            ps.setString(38, cdb.getSubcat());
        ...
        /*     ps.setInt(47, cdb.getOrno());
           ...
           ps.setDate(50, new java.sql.Date(cdb.getLrDate().getTime()));
            ...
            ps.setDouble(47, cdb.getGunybags());
            ps.setDouble(48, cdb.getBundles());
                //System.out.println("gunny bags");
            ps.setDouble(49, cdb.getPacking());
            ps.setDouble(50, cdb.getAutochrg());
            ps.setString(51, cdb.getCformstatus());
            ps.setString(52, cdb.getCf());
            ps.setDouble(53, cdb.getInvvalue());
...
ps.close();
         ...
         /*    if(cdb.getTaxtype().equalsIgnoreCase("CST 2%"))    {
            ...
            ps.close(); */
      //   &&l>0 && sales>0 && curbalup>0 && crbalu>0
            con.close();
            if(rows>0){
                FacesContext context = FacesContext.getCurrentInstance();
...
System.out.println("values are-----------------"+rows+"      "+j+"              "+k+"                         "+l); */
    ...
    public int StoringCurrBal() {
            ...
            try {
                ...
                rs.close();
                st.close();
                qry = "SELECT distinct(invvalue)  FROM  quotesales WHERE Billno =" +cdb.getBillno();
                System.out.println("" + qry);
                st=Conn_Statement();
                rs = st.executeQuery(qry);
                if (rs.next()) {
...
rs.close();
              ...
              /*   qry = "DELETE FROM quotesales WHERE Billno =" + cdb.getBillno();
                System.out.println("" + qry);
                st=Conn_Statement();
...
return l;
    ...
    public void onRowSelect1(SelectEvent event) {
        ...
        String qry =
            "select ITEMCODE , ITEMRATE,TAX,itemname,Disc   from ItemPurchase WHERE ITEMNAME  like '%" +
            itemname + "%'";
        System.out.println(qry);
        j = 0;
    ...
    public void onRowSelect2(SelectEvent event) {
       ...
       int z=0;
        ...
        String qry="select * from purchaseorder where reffno="+refno+"";
        try{
        st=Conn_Statement();
        rs=st.executeQuery(qry);
            while(rs.next()){
            ...
            //    cdb.setWaydate(rs.getDate(52));
                cdb.setGunybags(rs.getDouble(47));
                cdb.setBundles(rs.getDouble(48));
                cdb.setPacking(rs.getDouble(49));
                cdb.setAutochrg(rs.getDouble(50));
                cdb.setCformstatus(rs.getString(51));
                cdb.setCf(rs.getString(52));
                cdb.setInvvalue(rs.getDouble(53));
    ...
    public void update()  {
        ...
        String qry="delete from purchaseorder where Billno="+cdb.getBillno() +"";
    ...
    public void searchCust(){
        st=Conn_Statement();
        String qry="select distinct reffno,acname from purchaseorder";

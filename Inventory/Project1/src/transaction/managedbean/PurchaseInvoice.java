...
import transaction.beanpackage.Vendor;

import transaction.numbertoword.NumToWords;


@ManagedBean(name = "tpurchase", eager = true)
@ViewScoped
...
System.out.println("new currbalance is "+cdb.getCurrbal());
    ...
    public void onRowSelect(SelectEvent event) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                System.out.println("acccodde value is"+cdb.getAcccode()+"accno   value"+cdb.getAccno());
    ...
    public void bill() {
        ...
        st = Conn_Statement();
        String qry = "";
        qry = "SELECT max(salesid), max(Billno),max(reffno) FROM Vat_Purchase where cmpid='" + companyid + "'";
        try {
            rs = st.executeQuery(qry);
            if (rs.next()) {
...
System.out.println(new java.sql.Date(cdb.getInvdate().getTime()));
        try {
            ...
            String qry1="select * from Vat_Purchase where Billno="+cdb.getBillno() +"";
            st=Conn_Statement();
            ...
            String qry =
                "insert into Vat_Purchase values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
System.out.println("Row size is ---"+tdata.size());
           ...
           ps.setDate(50, new java.sql.Date(cdb.getLrDate().getTime()));
            ...
            if(cdb.getTaxtype().equalsIgnoreCase("CST 2%"))    {
                ...
                ps.setString(5,( cdb.getAcccode()).toString());
            ...
            ps=(PreparedStatement)con.prepareStatement("UPDATE Vendor SET currbal = " + cdb.getCurrbal()+ " WHERE acccode="+cdb.getAcccode() +"");
          
            crbalu=ps.executeUpdate();
          
            ps.close();
            con.close();
            System.out.println(rows+" "+curbalup+" "+crbalu);
            if(rows>0&&curbalup>0 && crbalu>0){
                FacesContext context = FacesContext.getCurrentInstance();
   ...
   // String iname = table.getValueAt(count, 1).toString();
    ...
    System.out.println("Row size is ---"+tdata.size());
           ...
           ps.setDate(50, new java.sql.Date(cdb.getLrDate().getTime()));
            ...
            ps=(PreparedStatement)con.prepareStatement("UPDATE Vendor SET currbal = " + cdb.getCurrbal()+ " WHERE acccode="+cdb.getAcccode() +"");
            System.out.println("UPDATE Vendor SET currbal = " + cdb.getCurrbal()+ " WHERE acccode="+cdb.getAcccode() +"");
...
System.out.println("values are-----------------"+rows+"      "+j+"              "+k+"                         "+l);
    ...
    public int StoringCurrBal() {
            ...
            try {
                ...
                rs.close();
                st.close();
                qry = "SELECT distinct(invvalue)  FROM  vat_purchase WHERE Billno =" +cdb.getBillno();
                System.out.println("" + qry);
                st=Conn_Statement();
                rs = st.executeQuery(qry);
                if (rs.next()) {
...
return l;
    ...
    public void onRowSelect1(SelectEvent event) {
        ...
        String qry =
            "select ITEMCODE , ITEMRATE,TAX,itemname,Disc,CATEGORY   from ItemPurchase WHERE ITEMNAME  like '%" +
            itemname + "%'";
        System.out.println(qry);
        j = 0;
    ...
    public void onRowSelect2(SelectEvent event) {
       ...
       int z=0;
        ...
        String qry="select * from Vat_Purchase where reffno="+refno+"";
        try{
        st=Conn_Statement();
        rs=st.executeQuery(qry);
            while(rs.next()){
    ...
    public void update()  {
        ...
        String qry="delete from Vat_Purchase where Billno="+cdb.getBillno() +"";
    ...
    public void searchCust(){
        st=Conn_Statement();
        String qry="select distinct reffno,acname from Vat_Purchase";

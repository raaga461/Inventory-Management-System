...
@ManagedBean(name = "sqou", eager = true)
@ViewScoped
...
//    } */
        ...
        try {
            ...
            if (cdb.getTaxtype() != null) {
                ...
                if (cdb.getTaxtype().contains("GEN")) {
                    ...
                    cdb.setInvno("QUOT/" + cdb.getRefno());
                    C_Form = "NO";
                }
    ...
    public void print(){
        ...
        SalesQuatationsPDF ob=new SalesQuatationsPDF();
       ob.execute(cdb.getBillno().toString(), cdb.getInvdate().toString(), nw);
    }
    ...
    public void bill() {
        ...
        st = Conn_Statement();
        String qry = "";
        qry = "SELECT max(salesid), max(Billno),max(reffno) FROM quotesales where cmpid='" + companyid + "'";
        try {
            rs = st.executeQuery(qry);
            if (rs.next()) {
...
System.out.println(new java.sql.Date(cdb.getInvdate().getTime()));
        try {
            ...
            String qry1="select * from quotesales where Billno="+cdb.getBillno() +"";
            st=Conn_Statement();
...
return l;
    ...
    public void onRowSelect1(SelectEvent event) {
        ...
        String qry =
            "select ITEMCODE , ITEMRATE,TAX,itemname,Disc   from ItemMaster WHERE ITEMNAME  like '%" +
            itemname + "%'";
        System.out.println(qry);
        j = 0;
    ...
    public void onRowSelect2(SelectEvent event) {
       ...
       int z=0;
        ...
        String qry="select * from quotesales where reffno="+refno+"";
        try{
        st=Conn_Statement();
        rs=st.executeQuery(qry);
            while(rs.next()){
    ...
    public void update()  {
        ...
        String qry="delete from quotesales where Billno="+cdb.getBillno() +"";
    ...
    public void searchCust(){
        st=Conn_Statement();
        String qry="select distinct reffno,acname from quotesales";

...
@ManagedBean(name = "cnote", eager = true)
@ViewScoped
...
System.out.println("new currbalance is "+cdb.getCurrbal());
    ...
    public void bill() {
       ...
       st = Conn_Statement();
           String qry = "";
           qry = "SELECT max(salesid), max(Billno),max(reffno) FROM CreditNoteSales where cmpid=" + companyid + "";
           try {
               rs = st.executeQuery(qry);
               if (rs.next()) {
...
System.out.println(new java.sql.Date(cdb.getInvdate().getTime()));
        try {
            ...
            String qry1="select * from CreditNoteSales where Billno="+cdb.getBillno() +"";
            st=Conn_Statement();
            ...
            String qry =
                "insert into creditnotesales values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
System.out.println("Row size is ---"+tdata.size());
           ...
           ps.setDate(50, new java.sql.Date(cdb.getLrDate().getTime()));
            ...
            ps.setString(70, tdata.get(i).getAproduct());
...
System.out.println("values are-----------------"+rows+"      "+j+"              "+k+"                         "+l);
    ...
    public int StoringCurrBal() {
            ...
            try {
                ...
                rs.close();
                st.close();
                qry = "SELECT distinct(invvalue)  FROM  creditnotesales WHERE Billno =" +cdb.getBillno();
                System.out.println("" + qry);
                st=Conn_Statement();
                rs = st.executeQuery(qry);
                if (rs.next()) {
...
rs.close();
               ...
               /*  qry = "DELETE FROM creditnotesales WHERE Billno =" + cdb.getBillno();
                System.out.println("" + qry);
                st=Conn_Statement();
...
return l;
    ...
    public void onRowSelect2(SelectEvent event) {
       ...
       int z=0;
        ...
        String qry="select * from creditnotesales where reffno="+refno+"";
        try{
        st=Conn_Statement();
        rs=st.executeQuery(qry);
            while(rs.next()){
             ...
             //   searchttb.seti(rs.getString(69));
                searchttb.setAproduct(rs.getString(70));
    ...
    public void update()  {
        ...
        String qry="delete from creditnotesales where Billno="+cdb.getBillno() +"";
    ...
    public void searchCust(){
        st=Conn_Statement();
        String qry="select distinct reffno,acname from creditnotesales";
